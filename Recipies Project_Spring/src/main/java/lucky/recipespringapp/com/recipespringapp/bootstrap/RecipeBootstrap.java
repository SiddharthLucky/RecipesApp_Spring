...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public RecipeBootstrap(CategoryRepository categoryRepository, RecipeRepository recipeRepository, UOMRepository unitOfMeasureRepository)
    {
    ...
    private List<Recipe> getRecipes()
    {
        ...
        //Can be used to throw exceptions at RunTime.
        //get UOMs
        Optional<UOM> eachUomOptional = unitOfMeasureRepository.findByDescription("Each");

        if(!eachUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UOM> tableSpoonUomOptional = unitOfMeasureRepository.findByDescription("Tablespoon");

        if(!tableSpoonUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UOM> teaSpoonUomOptional = unitOfMeasureRepository.findByDescription("Teaspoon");

        if(!teaSpoonUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UOM> dashUomOptional = unitOfMeasureRepository.findByDescription("Dash");

        if(!dashUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UOM> pintUomOptional = unitOfMeasureRepository.findByDescription("Pint");

        if(!pintUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UOM> cupsUomOptional = unitOfMeasureRepository.findByDescription("Cup");

        if(!cupsUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        //Checking to see if the optionals are present.
        //get optionals
        UOM eachUom = eachUomOptional.get();
        UOM tableSpoonUom = tableSpoonUomOptional.get();
        UOM teaspoonUom = tableSpoonUomOptional.get();
